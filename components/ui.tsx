...
export function Button({ variant = "primary", className = "", ...props }: ButtonProps) {
  ...
  const variants = {
    primary: "bg-gold py-3.5 text-plum hover:bg-gold-soft disabled:hover:bg-gold",
    ghost: "border border-gold/50 py-3 text-gold hover:bg-gold/10",
  };
  return (
...
export function TextInput({ className = "", ...props }: InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      className={`min-w-0 flex-1 rounded-lg border border-lilac/35 bg-plum-deep/60 px-3 py-2.5 text-base text-cream outline-none focus:border-gold ${className}`}
      {...props}
    />
  );
}
...
export function IconButton({ className = "", ...props }: ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      type="button"
      className={`w-11 shrink-0 rounded-lg border border-lilac/35 text-lg text-lilac transition enabled:hover:border-gold enabled:hover:text-gold disabled:opacity-30 ${className}`}
      {...props}
    />
  );
}
    ...
    <ol>
      {rows.map((row, index) => {
        ...
        return (
          <li
            key={row.name}
            className={`flex items-baseline gap-2.5 py-1.5 text-[17px] ${leading ? "text-gold" : ""}`}
          >
            <span className="w-[18px] shrink-0 text-[13px] text-lilac">{index + 1}</span>
