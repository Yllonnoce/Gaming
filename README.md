...
- **Audio knobs.** Every join link carries `&mediasettings` so each person gets
  ...
  than only a director. Everyone starts at `&audiogain=50` — phones inches from
  mouths run hot — and the room page's *Mic level* slider adjusts that,
  remembered per phone and clamped to 10–200% so it can never reach the `0`
  that mutes a guest until a director unmutes them. During the call the same
  slider applies live through `{function: "eval", value: "changeMainGain(n)"}`
  ([`commands.ts`](apps/noisy-room/commands.ts)) — the function VDO.Ninja's
  own settings slider calls, since the iframe API has no gain command. The
  string is pinned by a test because it is an undocumented dependency.
- **Device menus.** `{getDeviceList}` fills Microphone and Speaker menus in
  the call; switching uses `changeAudioDeviceById(id)` (the function the
  director's remote control uses) and the first-class
  `{changeAudioOutputDevice}`. Which devices are live is read back from the
  engine's audio track and sink. Device ids are validated against a strict
  alphabet before they go anywhere near the eval command.
- **We serve our own copy of Comms** at [`public/comms.html`](public/comms.html),
  generated by [`scripts/update-comms.mjs`](scripts/update-comms.mjs)
  (`npm run update-comms`). Comms has no switch for the `group: ` prefix it
  puts on every button, so the script fetches upstream and applies a short,
  counted list of string patches — that one, plus pointing its relative asset
  and iframe paths back at `vdo.ninja`. If upstream moves the code, the
  counts fail and the script refuses rather than shipping a half-patched page.
  The audio engine is still loaded from `vdo.ninja`; only the button shell is
  ours. `NEXT_PUBLIC_COMMS_URL=https://comms.cam/` switches back to upstream.
- **Licence note.** Comms is AGPL-3.0, unlike the rest of this repository
  (MIT). AGPL asks that a modified copy served to users be publicly available;
  the served file is its own source, the patch list is the modification, and
  both are in this repository, which the file's header comment points to.
